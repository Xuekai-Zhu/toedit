...
def merge_sampled_files(input_dir, output_dir, chunk_size=1000000, target_tokens=1e9):
    ...
    elif "orca" in input_dir.lower():
        sample_lines = [item["question"] + " " + item["response"] for item in dataset.select(range(1000))]
    ...
    for item in tqdm(selected_datasets):
        if "Arithmo-Data" in input_dir:
            ...
            text_line = json.dumps({"text": item["question"] + " " + item["response"]}) + "\n"
...
if __name__ == "__main__":
    input_dir = "data/Open-Orca/OpenOrca"
    output_dir = "data/Open-Orca/OpenOrca-1B"
