...
def merge_sampled_files(input_file_list, output_dir, chunk_size=1000000):
    ...
    for input_files in input_file_list:
        ...
        if "finance" in input_files[0].lower():
            selected_datasets = sample_fixed_tokens(one_source_lines, target_tokens=5e8)
            all_sampled_lines.extend(selected_datasets)
        elif "orca" in input_files[0].lower():
            selected_datasets = sample_fixed_tokens(one_source_lines, target_tokens=1e9)
            all_sampled_lines.extend(selected_datasets)
...
if __name__ == "__main__":
    source_1 = "data/finance_all/finance_up_revise_Llama-3-8B-Instruct"
    source_files_1 = search_datasets(source_1)
    
    source_2 = "data/Open-Orca/OpenOrca-1B"
    source_files_2 = search_datasets(source_2)
    
    output_dir = "data/finance_all/finance_revised_orca_1_2"
