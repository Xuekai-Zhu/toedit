...
def count_tokens(request_output, file_path, tokenizer, source_in_texts):
    up_threshold = 0.99
    low_threshold = 0.001
    count_num = 0
    # seconda_threshold = 0.000001
    ...
    with open(file_path, 'w') as json_file:
        # for output_res in outputs_to_save:
        generated_text = json.dumps({"threshold":up_threshold, "tokens":count_num}, ensure_ascii=False)
        json_file.write(generated_text + "\n")
        
def count_all_probalities_tokens(request_output, file_path, tokenizer, source_in_texts):
    up_threshold = 0.99
    low_threshold = 0.001
    count_num = 0
    ...
    for i, output in enumerate(request_output):
        ...
        for i_index, prompt_logprob in enumerate(output.prompt_logprobs):
            ...
            if prompt_logprob is None:
                ...
                # Determine which probability interval the current prob falls into
                if 0 <= prob < 0.1:
                    probability_intervals["[0.0-0.1]"] += 1
                elif 0.1 <= prob < 0.2:
                    probability_intervals["[0.1-0.2]"] += 1
                elif 0.2 <= prob < 0.3:
                    probability_intervals["[0.2-0.3]"] += 1
                elif 0.3 <= prob < 0.4:
                    probability_intervals["[0.3-0.4]"] += 1
                elif 0.4 <= prob < 0.5:
                    probability_intervals["[0.4-0.5]"] += 1
                elif 0.5 <= prob < 0.6:
                    probability_intervals["[0.5-0.6]"] += 1
                elif 0.6 <= prob < 0.7:
                    probability_intervals["[0.6-0.7]"] += 1
                elif 0.7 <= prob < 0.8:
                    probability_intervals["[0.7-0.8]"] += 1
                elif 0.8 <= prob < 0.9:
                    probability_intervals["[0.8-0.9]"] += 1
                elif 0.9 <= prob <= 1.0:
                    probability_intervals["[0.9-1.0]"] += 1
...
def main():
    ...
    for i, file_path in enumerate(files_to_process):
        ...
        for start in tqdm(range(0, len(out_texts), args.batch_size), desc=f"Processing batch"):
            ...
            count_tokens(outputs, results_file, tokenizer, batch_intexts)
